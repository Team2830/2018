...
public class DriveDistance extends Command {
    ...
    protected void execute() {
        ...
        double v;
//        double d = Robot.driveTrain.getInchesFromPulses(m_distance);
       	double x = Robot.driveTrain.getDistance();
       	double vMin = .2;
       	double xRamp = Math.min(18,m_distance/2);
       	double xBrake = m_distance-Math.min(18,m_distance/2);
       	if (x>xBrake)
       		v=m_speed+((vMin-m_speed)/(m_distance-xBrake))*(2*(x-m_distance));
       	else if (x<xRamp)
       		v=vMin+((m_speed-vMin)/xRamp)*x;
       	else
      		v=m_speed;
       	SmartDashboard.putNumber("Ramp Velocity", v);
       	SmartDashboard.putNumber("Distance Driven", x);
